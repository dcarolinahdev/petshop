...
async function fecthApi() {
  try {
    ...
    medicines = response.products.filter((item) => item.tipo==="Medicamento");
    console.log(medicines);

    refresh_cart_in_navbar();
    printCards(medicines);
...
async function filterData() {
  try {
    ...
    let url = `https://pro-talento.up.railway.app/api/mindy/products`;
    url += `?tipo=medicamento&orden=${value_selected}`;
    if (texto.length != 0) {
      url += `&nombre=${texto}`;
    }
    console.log(url);
    let response = await fetch(url);
    response = await response.json();
    let medFiltered = response.products;

    if (medFiltered.length == 0) {
      printEmpty();
    } else {
      printCards(medFiltered);
    }
  } catch (error) {
    console.log(error);
  }
...
function printCards(array) {
    const containerCategory = document.getElementById('container-medicines');
    containerCategory.innerHTML = '';

    for (let i = 0; i < array.length; i++) {

        let div = createMedCard(array[i]);
...
function printEmpty() {
    const containerCards = document.getElementById('container-medicines');
...
function createMedCard(producto) {
    ...
    div.innerHTML = `
        <img src="${producto.imagen}" alt="Medicine">
